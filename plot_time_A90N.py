...
#for j in range(3):
    # Extract columns
bulkra = df.iloc[:, 37].values  # Column 15
bulkdec = df.iloc[:, 38].values  # Column 16
bulk = df.iloc[:, 22].values   # Column 2
...
for i in range(len(bulk)):
    ...
    if time_mat[int(n * bulkra[i] /(2* np.pi) - 1), int(n * (bulkdec[i]) /(0.5*np.pi) - 1)]==0:
        time_mat[int(n * bulkra[i] / (2* np.pi)  - 1), int(n * (bulkdec[i]) / (0.5*np.pi) - 1)] = bulk[i]
